...
from sqlalchemy import Boolean, DateTime, ForeignKey, func, Index, JSON, String, Text
...
class Event(Base):
    ...
    # Correlation Engine: UUID инцидента к которому принадлежит событие
    incident_id: Mapped[str | None] = mapped_column(String(36), nullable=True, index=True)

    # Pipeline Latency Monitoring: время приёма события API
    ingested_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True,
        server_default=func.now(), comment="Время приёма события API"
    )

    # Pipeline Latency Monitoring: время отправки алерта
    alert_sent_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="Время отправки алерта"
    )
